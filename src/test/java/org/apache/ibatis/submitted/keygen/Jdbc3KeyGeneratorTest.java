    ...
    public void shouldFailIfKeyPropertyIsInvalid_WrongParamName2() throws Exception { // add by 芋艿
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            try {
                CountryMapper mapper = sqlSession.getMapper(CountryMapper.class);
                Country country = new Country("China", "CN");
//                mapper.insertMultiParams_keyPropertyWithWrongParamName2(country, 1);
...
//                                + "Specified key properties are [bogus.id] and available parameters are [");
    ...
    @Test
    public void shouldAssignMultipleGeneratedKeysToABean2() throws Exception {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            try {
                CountryMapper mapper = sqlSession.getMapper(CountryMapper.class);
                Map<String, Object> planet = new MapperMethod.ParamMap<>();
                planet.put("name", "pluto");
//                mapper.insertPlanet2(planet);
//                mapper.insertPlanet2("pluto", "123");
